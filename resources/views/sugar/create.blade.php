...
<html lang="ja">
  ...
  <a href="/sugar/{{ Auth::user()->id }}">list</a>
      ...
      @endif
        ...
        <form action="/sugar/create" method="post" id="product">
          @csrf
          <div class="input">
            <span>食べた商品</span> : 
            <select name="product_id" id="product_id" onchange='changeSelectBox()'>
              ...
              @foreach($products as $product)
                <option value="{{$product->product_id}}" required>{{$product->name}}</option>
              @endforeach
            </select><br/>
          </div>
          <div class="input">
            <span>数量</span> : <input type="number" name="amount" id="amount" step="0.1" min="0" placeholder="食べた数（個）を入力してください" required onchange='changeSelectBox()'><br/>
          </div>
          <div class="input">
            <span>日付</span> : <input type="date" name="date" required>
          </div>
